...
- [x] Install zsh, neovim in all test containers; add functional post-bootstrap checks (`zsh -ilc true`, `vim --not-a-term -c q`, `nvim --headless -c q`). Still TODO: tmux (`tmux -f ~/.tmux.conf -C kill-server`).
